...
(ert-deftest bencode-string ()
  ...
  (let ((utf-8 (bencode-encode "naïvety"))
        (latin-1 (bencode-encode "naïvety" :coding-system 'latin-1)))
...
(ert-deftest bencode-dict ()
  ...
  (let* ((table #s(hash-table test equal data ("α" 0 "β" 1 "ω" 2)))
         ...
         (greek (bencode-encode table :coding-system 'iso-8859-7))
         (greek-plist (bencode-encode plist :coding-system 'iso-8859-7)))
    (should (not (multibyte-string-p utf-8)))
...
(ert-deftest bencode-decode-string ()
  ...
  (should (equal (bencode-decode "7:na\xefvety" :coding-system 'latin-1)
                 "naïvety")))
...
(ert-deftest bencode-decode ()
  ...
  (should (equal (bencode-decode "li0ei1ei2ee" :list-type 'vector) [0 1 2])))
